...
const formatCurrency = (value) => {
  const numericValue = parseFloat(value); // Converte para número
...
const ReservationCard = ({ reserva }) => {
  ...
  return (
    <>
                  ...
                  {rooms.slice(3).map((quarto) => (
                    <Box key={quarto.id} sx={{ mb: 1.5 }}>
                      ...
                      <Typography variant="body2" color="text.secondary" sx={{ pl: '26px' }}>
                        Diária: {formatCurrency(quarto.daily_rate)} | Total: {formatCurrency(quarto.total)}
                      </Typography>
                    </Box>
                  ))}
                </Collapse>
      ...
      <ReservationDialog reserva={reserva} open={dialogOpen} onClose={handleCloseDialog} />
    </>
  );
};
